...
function resolveDependencies(directory)
{
    ...
    dependencyLinkPromises.push(fsReadFile(path.join(directory, 'package.json'), 'utf8').then((package) => {
        ...
        {
            var promises = [];
            package.localDependencies.forEach((localDependency) => {
                var command = this.tool === 'yarn' ? "yarn link \"" + localDependency + "\"": "npm link " + localDependency;
                promises.push(childProcessExec(command, {'cwd': directory}));
            });
...
function link(dependents, tool)
{
    dependencyLinkPromises = [];
    return new Promise((resolve, reject) => {
        Promise.all(dependents.map((directory) => {
            return actOnModules(directory, resolveDependencies.bind({'tool': tool}));
        })).then(() => {
